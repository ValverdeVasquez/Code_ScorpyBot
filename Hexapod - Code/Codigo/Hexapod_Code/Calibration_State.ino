...
void calibrationState(){
  ...
  if(!legsUp){
    ...
    for (int i = 0; i < 6; i++){
      float nextX = min(currentPoints[i].x + 0.5, static_cast<double>(targetCalibration.x));
      float nextY = min(currentPoints[i].y + 0.5, static_cast<double>(targetCalibration.y));
      float nextZ = min(currentPoints[i].z + 0.5, static_cast<double>(targetCalibration.z));
